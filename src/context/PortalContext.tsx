...
export const PortalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateFacultyProfile = (facultyId: string, updatedData: Partial<Faculty>) => {
    ...
    // Persist to TiDB Cloud backend
    fetch(`/api/faculty/${encodeURIComponent(facultyId)}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'x-user-role': currentUser?.role || 'admin',
        'x-user-id': (currentUser?.data as any)?.id || 'ADM001',
      },
      body: JSON.stringify(updatedData),
    })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Faculty update response:', data.message);
      })
      .catch((err) => console.error('Error saving faculty to TiDB:', err));
  };
  ...
  const addFaculty = (newFaculty: Faculty) => {
    ...
    fetch('/api/faculty', {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Add faculty response:', data.message);
      })
      .catch((err) => console.error('Error creating faculty in TiDB:', err));
  };
  ...
  const deleteFaculty = (facultyId: string) => {
    ...
    }).catch((err) => console.error('Error deleting faculty from TiDB:', err));
  };
  ...
  const updateHODProfile = (updatedData: Partial<HOD>) => {
    ...
    fetch('/api/hod', {
      ...
      body: JSON.stringify(updatedData),
    }).catch((err) => console.error('Error saving HOD to TiDB:', err));
  };
  ...
  const updateHOD = (hodId: string, updatedData: Partial<HOD>) => {
    ...
    fetch('/api/hod', {
      ...
      body: JSON.stringify(updatedData),
    }).catch((err) => console.error('Error updating HOD in TiDB:', err));
  };
  ...
  const updateStudentProfile = (regNo: string, updatedData: Partial<Student>) => {
    ...
    fetch(`/api/students/${encodeURIComponent(regNo)}`, {
      ...
      headers: {
        'Content-Type': 'application/json',
        'x-user-role': currentUser?.role || 'admin',
        'x-user-id': (currentUser?.data as any)?.id || (currentUser?.data as any)?.regNo || 'ADM001',
      },
      body: JSON.stringify(updatedData),
    })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Student update response:', data.message);
      })
      .catch((err) => console.error('Error saving student to TiDB:', err));
  };
  ...
  const addStudent = (newStudent: Student) => {
    ...
    fetch('/api/students', {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Add student response:', data.message);
      })
      .catch((err) => console.error('Error creating student in TiDB:', err));
  };
  ...
  const deleteStudent = (regNo: string) => {
    ...
    }).catch((err) => console.error('Error deleting student from TiDB:', err));
  };
  ...
  const addMentorMeetingNote = (noteData: Omit<MentorMeetingNote, 'id'>) => {
    ...
    fetch(`/api/students/${encodeURIComponent(noteData.regNo)}/mentor-notes`, {
      ...
      headers: {
        ...
        'x-user-id': (currentUser?.data as any)?.id || 'FAC001',
      },
      body: JSON.stringify(noteData),
    }).catch((err) => console.error('Error saving mentor note to TiDB:', err));
  };
  ...
  const saveAttendanceBatch = (newRecords: AttendanceRecord[]) => {
    ...
    fetch('/api/attendance/batch', {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Attendance save response:', data.message);
      })
      .catch((err) => console.error('Error saving attendance to TiDB:', err));
  };
  ...
  const updateStudentMarks = (
    ...
    fetch(`/api/marks/${encodeURIComponent(regNo)}/${encodeURIComponent(subjectCode)}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) console.warn('Marks save response:', data.message);
      })
      .catch((err) => console.error('Error saving marks to TiDB:', err));
  };
  ...
  const updateAssignmentStatus = (
    ...
    }).catch((err) => console.error('Error updating assignment in TiDB:', err));
  };
  ...
  const updateStudentPerformance = (
    ...
    }).catch((err) => console.error('Error updating performance in TiDB:', err));
  };
  ...
  const updateAdminProfile = (updatedData: Partial<Admin>) => {
    ...
    fetch('/api/auth/profile', {
      ...
      body: JSON.stringify(updatedData),
    }).catch((err) => console.error('Error updating admin profile in TiDB:', err));
  };
  ...
  const updateFeePayment = (feeId: string, amount: number) => {
    ...
    }).catch((err) => console.error('Error saving fee payment to TiDB:', err));
  };
  ...
  const addFeeRecord = (newFee: FeeRecord) => {
    ...
    }).catch((err) => console.error('Error adding fee to TiDB:', err));
  };
  ...
  const deleteFeeRecord = (feeId: string) => {
    ...
    }).catch((err) => console.error('Error deleting fee from TiDB:', err));
  };
