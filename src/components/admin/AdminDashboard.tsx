...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const exportStudentsCSV = () => {
    ...
    const rows = students
      .map(
        (s) =>
          `"${s.regNo}","${s.name}",${s.year},"${s.section}","${s.department}","${s.email}","${s.phone}",${s.cgpa},${s.overallAttendance?.percentage ?? s.attendancePercentage ?? 0}%,"${s.accountStatus}"`
      )
      .join('\n');
  ...
  const avgAttendance = students.length > 0 ? Math.round(students.reduce((acc, s) => acc + (s.overallAttendance?.percentage ?? s.attendancePercentage ?? 0), 0) / students.length) : 0;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
              ...
              <div className="text-[11px] text-slate-500 mt-1">
                {students.filter((s) => (s.overallAttendance?.percentage ?? s.attendancePercentage ?? 0) < 75).length} students &lt;75% threshold
              </div>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {subjects.map((sub) => (
                  <tr key={sub.code} className="hover:bg-slate-50/70">
                    ...
                    <td className="py-2.5 px-3 text-slate-700 font-medium">{sub.facultyName || sub.faculty}</td>
                    <td className="py-2.5 px-3 text-right">
                      <button
                        type="button"
            ...
            <form
              onSubmit={(e) => {
                ...
                if (departments.some((d) => d.id === deptForm.id)) {
                  ...
                  addDepartment({
                    ...deptForm,
                    id: deptForm.id || `dept-${deptForm.code.toLowerCase()}`,
                    hodEmail: `${deptForm.code.toLowerCase()}.hod@college.edu`,
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-5 shadow-xl border border-slate-200 space-y-4">
            ...
            <form
              onSubmit={(e) => {
                ...
                addSubject({
                  ...subjectForm,
                  facultyName: subjectForm.faculty || 'Faculty In-Charge',
                });
