...
import { SubjectMarks, SubjectAttendance, Student } from '../../types';
  ...
  const [selectedStudentRegNo, setSelectedStudentRegNo] = useState<string>(
    targetStudentRegNo ||
      (currentUser?.role === 'student'
        ? (currentUser.data as Student).regNo
                ...
                <tbody className="divide-y divide-slate-200">
                  {departments.map((dept) => (
                    <tr key={dept.id}>
                      ...
                      <td className="p-2 text-slate-700">{dept.hodName || dept.hod}</td>
                      <td className="p-2 text-center font-semibold">{dept.totalFaculty ?? dept.facultyCount ?? 0}</td>
                      <td className="p-2 text-center font-semibold">{dept.totalStudents ?? dept.studentCount ?? 0}</td>
