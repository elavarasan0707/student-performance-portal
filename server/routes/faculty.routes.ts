...
router.post('/', requireAuth, requireRole('admin', 'hod'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  await repo.persistFaculty(newFaculty);
  ...
  res.status(201).json({ success: true, message: 'Faculty appointed and saved successfully', data: newFaculty });
});
...
router.put('/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Persist updated faculty to TiDB Cloud
  await repo.persistFaculty(faculty);
  ...
  res.json({ success: true, message: 'Faculty profile updated and saved', data: faculty });
});
...
router.delete('/:id', requireAuth, requireRole('admin'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  await repo.removeFaculty(id);

  repo.logAudit(
    'FACULTY_DELETED',
    req.user!.email,
    'admin',
    `De-registered faculty ${facultyName} (${id})`,
    req.ip
  );

  res.json({ success: true, message: 'Faculty member removed from institution roll and database' });
});
