...
router.put('/', requireAuth, requireRole('hod', 'admin'), async (req: AuthenticatedRequest, res: Response) => {
  Object.assign(repo.hod, req.body);
  await repo.persistHOD(repo.hod);
  repo.logAudit(
    'HOD_PROFILE_UPDATED',
    req.user?.email || 'hod@college.edu',
    req.user?.role || 'hod',
    `Updated HOD profile for ${repo.hod.name}`,
    req.ip
  );
  res.json({ success: true, message: 'HOD profile updated and saved', data: repo.hod });
});
