...
async function handleBatchAttendance(req: AuthenticatedRequest, res: Response) {
  ...
  // Persist to TiDB Cloud & recalculate affected students
  if (savedRecords.length > 0) {
    await repo.persistAttendanceBatch(savedRecords);
  }
  ...
  res.json({
    success: true,
    message: `Successfully processed and saved ${savedRecords.length} attendance entries`,
    count: savedRecords.length,
    data: savedRecords,
  });
}

// POST /api/attendance/batch - Mark/Update batch attendance
router.post('/batch', requireAuth, requireRole('admin', 'hod', 'faculty'), handleBatchAttendance);

// POST /api/attendance - Single or batch attendance
router.post('/', requireAuth, requireRole('admin', 'hod', 'faculty'), handleBatchAttendance);
