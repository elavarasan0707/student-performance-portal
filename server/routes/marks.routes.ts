...
router.put('/:regNo/:subjectCode', requireAuth, requireRole('admin', 'hod', 'faculty'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Persist updated student to TiDB Cloud
  await repo.persistStudent(student);
  ...
  res.json({
    success: true,
    message: 'Academic marks updated and saved successfully',
...
// POST /api/marks/batch - Batch update marks for section
router.post('/batch', requireAuth, requireRole('admin', 'hod', 'faculty'), async (req: AuthenticatedRequest, res: Response) => {
  const { updates } = req.body;
  if (!Array.isArray(updates) || updates.length === 0) {
    return res.status(400).json({ success: false, message: 'Valid array of updates is required' });
  }

  const updatedStudents: string[] = [];

  for (const item of updates) {
    const { regNo, subjectCode, internal1, internal2, modelExam, assignment } = item;
    const student = repo.students.find((s) => s.regNo.toUpperCase() === String(regNo).toUpperCase());
    if (!student) continue;

    if (!student.marks) student.marks = {};
    const existing = student.marks[subjectCode] || {
      subjectCode,
      subjectName: subjectCode,
      internal1: 0,
      internal2: 0,
    ...
    const i1 = internal1 !== undefined ? Number(internal1) : existing.internal1;
    const i2 = internal2 !== undefined ? Number(internal2) : existing.internal2;
    const model = modelExam !== undefined ? Number(modelExam) : existing.modelExam;
    const assign = assignment !== undefined ? Number(assignment) : existing.assignment;

    const internalAvg = (i1 + i2) / 2;
    const scaledInternal = (internalAvg / 50) * 20;
    const scaledModel = (model / 100) * 70;
    const scaledAssignment = (assign / 10) * 10;
    const totalPercentage = Math.round((scaledInternal + scaledModel + scaledAssignment) * 10) / 10;
    const grade = calculateGrade(totalPercentage);
    ...
    await repo.persistStudent(student);
    updatedStudents.push(student.regNo);
  }
  ...
  repo.logAudit(
    ...
    req.user!.email,
    req.user!.role,
    `Batch updated marks for ${updatedStudents.length} students`,
    req.ip
  );

  res.json({
    success: true,
    message: `Batch marks updated for ${updatedStudents.length} students`,
...
router.put('/assignment/:regNo/:assignmentId', requireAuth, requireRole('admin', 'hod', 'faculty'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  res.json({ success: true, message: 'Assignment evaluated and saved', data: assignment });
});
